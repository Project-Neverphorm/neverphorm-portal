  ...
  return (
    <div className="min-h-screen bg-black text-white">
      ...
      <div className="flex">
        ...
        <aside className="w-72 shrink-0 border-r border-neutral-800 px-6 py-10 min-h-screen">
          <div className="flex items-center justify-between mb-6">
            <h2 className="text-sm uppercase tracking-wide text-neutral-400">Team</h2>
          ...
          <div className="space-y-6">
            <div>
              <div className="flex items-center gap-3 mb-2">
                <div className="w-10 h-10 rounded-full bg-cyan-400 text-black font-bold flex items-center justify-center shrink-0">
                  CM
                </div>
                <div>
                  <p className="font-semibold text-sm">Cody</p>
                  <p className="text-xs text-neutral-400">Studio Head / Creative Director</p>
                </div>
              </div>
              <p className="text-xs text-green-400 mb-1">In Motion — 5m ago</p>
              <p className="text-xs text-neutral-400 mb-1">Lvl 0 · {personalXP} / 500 XP</p>
              <div className="w-full h-1.5 bg-neutral-800 rounded-full overflow-hidden">
                <div className="h-full bg-cyan-400" style={{ width: `${Math.min((personalXP / 500) * 100, 100)}%` }} />
              </div>
            </div>
          </div>

          <button className="mt-8 w-full text-sm text-neutral-400 hover:text-white border border-neutral-700 rounded px-4 py-2">
            + Invite Member
          </button>
        </aside>
        ...
        <main className="flex-1 px-10 py-10">
          ...
          <div className="mb-10">
            ...
            <p className="text-neutral-400 text-sm mt-1">Here&apos;s where things stand.</p>
          </div>

          {/* Studio Progress - full width */}
          <div className="border-b border-neutral-800 pb-10 mb-10">
            <div className="flex items-center justify-between mb-3">
              <h2 className="text-sm uppercase tracking-wide text-neutral-400">Studio Progress</h2>
            ...
            <div className="w-full h-2 bg-neutral-800 rounded-full overflow-hidden">
              <div className="h-full bg-cyan-400" style={{ width: `${Math.min((studioXP / 2500) * 100, 100)}%` }} />
            </div>
            <p className="text-sm text-neutral-400 mt-2">{studioXP} / 2500 XP</p>
          </div>

          {/* Three-column stats - spread across full main width */}
          <div className="grid grid-cols-3 gap-10 border-b border-neutral-800 pb-10 mb-10">
            <div>
              <p className="text-xs uppercase tracking-wide text-neutral-400 mb-2">Next Goal</p>
              <p className="font-semibold">Complete Current Task Boxes</p>
              <p className="text-sm text-neutral-400 mt-1">+2,500 XP Team Goal</p>
            </div>
            <div>
              <p className="text-xs uppercase tracking-wide text-neutral-400 mb-2">Level Up Reward</p>
              <p className="font-semibold">Studio Level 1</p>
              <p className="text-sm text-cyan-400 mt-1">✓ Custom Studio Mug</p>
            </div>
            <div>
              <p className="text-xs uppercase tracking-wide text-neutral-400 mb-2">Next Unlock</p>
              <p className="font-semibold">Reward Slot</p>
              <p className="text-sm text-neutral-400 mt-1">Unlock after Level 1</p>
            </div>
          </div>
          ...
          {/* Tasks + Team Task Boxes + Responsibilities */}
            ...
            <div className="col-span-2">
                ...
                <h2 className="text-sm uppercase tracking-wide text-neutral-400">My Current Tasks</h2>
                <span className="text-xs text-neutral-400">🎯 Target: Ongoing</span>
                </div>
                <p className="text-2xl font-bold mb-1">Current Tasks</p>
                <p className="text-cyan-400 text-sm mb-3">Assigned to Cody</p>

                <div className="w-full h-2 bg-neutral-800 rounded-full overflow-hidden mb-2">
                <div className="h-full bg-cyan-400" style={{ width: `${Math.min((personalXP / 500) * 100, 100)}%` }} />
                </div>
                <p className="text-sm text-neutral-400 mb-6">{personalXP} / 500 XP · {tasks.filter(t => t.done).length} / {tasks.length} tasks complete</p>
                ...
                {tasks.map((task) => (
                    ...
                    <div className="flex items-center gap-3">
                        ...
                        <p className={`text-sm ${task.done ? 'text-neutral-500 line-through' : ''}`}>
                        {task.name}
                        </p>
                    </div>
                    <div className="flex items-center gap-4">
                        <span className={`text-sm font-semibold ${task.done ? 'text-neutral-600' : 'text-cyan-400'}`}>
                        +{task.xp} XP
                        </span>
                        <button
            ...
            {/* Responsibilities */}
            <div>
                <h2 className="text-sm uppercase tracking-wide text-neutral-400 mb-3">Responsibilities</h2>
            ...
            {/* Team Task Boxes */}
            <div>
                <h2 className="text-sm uppercase tracking-wide text-neutral-400 mb-3">Team Task Boxes</h2>
                <div className="border-l-2 border-cyan-400 pl-4">
                <div className="flex items-center justify-between">
                    <p className="font-semibold">Cody</p>
                    <span className="text-sm text-neutral-400">{personalXP}/500</span>
                </div>
                <p className="text-cyan-400 text-sm">Current Tasks</p>
                <p className="text-xs text-neutral-400 mb-2">🎯 Target: Ongoing</p>
                <div className="w-full h-2 bg-neutral-800 rounded-full overflow-hidden mb-1">
                    <div className="h-full bg-cyan-400" style={{ width: `${Math.min((personalXP / 500) * 100, 100)}%` }} />
                </div>
                <p className="text-xs text-neutral-400">{tasks.filter(t => t.done).length} / {tasks.length} tasks complete</p>
                </div>
            </div>
